...
Joosy.Modules.Events =
  ...
  # @param [Boolean] strict             Call has no effect if true and the given pair
  #                                     of events and callback is already set up
  #
  wait: (events, callback, strict = false) ->
    events = @__splitEvents events
    @__validateEvents events

    return if strict && @__isBound(events, callback, true)
  ...
  #                                     of events and callback is already set up
  #
  bind: (events, callback, strict = false) ->
    events = @__splitEvents events
    @__validateEvents events

    return if strict && @__isBound(events, callback)
