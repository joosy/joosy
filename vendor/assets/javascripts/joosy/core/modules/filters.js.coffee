Joosy.Modules.Filters =
  ...
  __runBeforeLoads: (opts...) ->
    return true if !@__before_loads?.length > 0
  ...
  __runAfterLoads: (opts...) ->
    filter.apply(@, opts) for filter in @__after_loads if @__after_loads?

  __runAfterUnloads: (opts...) ->
    filter.apply(@, opts) for filter in @__after_unloads if @__after_unloads?